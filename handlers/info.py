...
async def temp_keeping(call: types.CallbackQuery) -> None:
    try:
        ...
        await call.message.answer(
            text=f'Передержка – ключевой момент в спасении бездомного животного. Если в Вашем доме есть место для кота или собаки, которые пока не нашли своего хозяина, Вы можете помочь, предоставив им временный дом! И это совсем не означает, что, взяв собаку или кота к себе, Вы берете на себя его дальнейшее содержание и пристройство. Кураторы животных всегда помогут всем необходимым, привезут и увезут в оговоренные с Вами сроки. Даже кратковременная передержка – это очень большая помощь!',
...
async def auto_help(call: types.CallbackQuery) -> None:
    try:
        ...
        user_info = bool(db.user_auto(call.from_user.id))

        await call.message.answer(
                text=f'Автопомощь – одно из важных направлений помощи бездомным животным. Ежедневно кому-то из наших животных нужно попасть на прием к врачу, переехать на передержку, поучаствовать в акциях и выставках, доехать к новому хозяину, но не всегда у волонтеров есть возможность транспортировать животное на общественном транспорте. Именно поэтому нам очень нужна Ваша помощь, уважаемые автовладельцы! Если у вас найдется немного свободного времени и желание помочь – проявите участие! Оставьте заявку и вам иногда будут приходить просьбы о помощи, на которые вы сможете при возможности откликнуться.',
                reply_markup=(kb.kb_auto_help, kb.kb_auto_help2)[user_info])


async def auto_help_reg_menu(call: types.CallbackQuery):
    try:
        await call.message.delete()
    finally:
        data = call.data.split('|')
        action = {'reg': (1, '<b>Спасибо, что предложили помощь!</b>\n<i>Иногда вам будут приходить уведомления с просьбами о помощи, на которые вы сможете откликнуться.</i>'), 'del' : (0, '<b>Заявка удалена.</b>\n<i>Вы не будете получать сообщения с просьбами об автопомощи.</i>')}
        db.change_user_auto_status(user_id=call.from_user.id, status=action[data[1]][0])
        await call.message.answer(text=action[data[1]][1], reply_markup=(kb.kb_auto_help, kb.kb_auto_help2)[action[data[1]][0]])
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(auto_help_reg_menu, lambda call: call.data.startswith('autoreg'))
